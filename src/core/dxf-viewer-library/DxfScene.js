...
export class DxfScene {
    ...
    async _FetchFonts(dxf) {
        ...
        const ProcessEntity = async (entity) => {
            ...
            if (entity.type === "TEXT" || entity.type === "ATTRIB" || entity.type === "ATTDEF") {
                ...
                ret = true
                const dim = this._CreateDimension(entity)
                if (dim) {
    ...
    _ProcessDxfEntity(entity, blockCtx = null) {
        ...
        for (const renderEntity of renderEntities) {
            renderEntity.dxfType = entity.type;
            renderEntity.dxfHandle = entity.handle;
    ...
    *_DecomposeMText(entity, blockCtx) {
    ...
    _CreateDimension(entity) {
        ...
        const styleResolver = valueName => {
            return this._GetDimStyleValue(valueName, entity, style);
        }

        const textWidthCalculator = (text, fontSize) => {
        ...
        if (type === 0 || type === 1) {
            if (!entity.linearOrAngularPoint1 || !entity.linearOrAngularPoint2 || 
                !entity.anchorPoint) {
                return null
            }

            dim = new LinearDimension({
            ...
            }, styleResolver, textWidthCalculator)

        } else if (type === 2 || type === 5) {
            if (!entity.anchorPoint || !entity.linearOrAngularPoint1 || 
                !entity.linearOrAngularPoint2 || !entity.arcPoint) {
                return null;
            }

            dim = new AngularDimension({
                center: new Vector2().copy(entity.anchorPoint),
                point1: new Vector2().copy(entity.linearOrAngularPoint1),
                point2: new Vector2().copy(entity.linearOrAngularPoint2),
                arcPoint: new Vector2().copy(entity.arcPoint),
                text: entity.text,
                textAnchor: entity.middleOfText ? new Vector2().copy(entity.middleOfText) : null,
                textRotation: entity.textRotation,
        ...
        if (!dim) {
            console.warn("Unsupported dimension type detected for " + entity.handle)
            return null;
        }
    ...
    *_DecomposeSpline(entity, blockCtx = null) {
    ...
    _ProcessPoints(entity, blockCtx = null) {
        const key = new BatchingKey(entity.layer, blockCtx?.name,
                                    ...
                                    entity.dxfHandle, entity.dxfType)
        const batch = this._GetBatch(key)
        for (const v of entity.vertices) {
    ...
    _ProcessLineSegments(entity, blockCtx = null) {
        ...
        const useExactBatching = entity.dxfType === "LINE" || 
                               entity.dxfType === "LWPOLYLINE" || 
                               entity.dxfType === "POLYLINE" || 
                               entity.dxfType === "ARC" || 
                               entity.dxfType === "CIRCLE" || 
                               entity.dxfType === "ELLIPSE" || 
                               entity.dxfType === "SPLINE" ||
                               entity.dxfType === "TEXT" ||
                               entity.dxfType === "MTEXT" ||
                               entity.dxfType === "ATTRIB";
        const key = new BatchingKey(entity.layer, blockCtx?.name,
                                    BatchingKey.GeometryType.LINES, entity.color, entity.lineType,
                                    useExactBatching ? entity.dxfHandle : null,
                                    useExactBatching ? entity.dxfType : null)
        const batch = this._GetBatch(key)
        for (const v of entity.vertices) {
            batch.PushVertex(this._TransformVertex(v, blockCtx))
        }
    ...
    _ProcessTriangles(entity, blockCtx = null) {
        ...
        const useExactBatching = entity.dxfType === "TEXT" || 
                               entity.dxfType === "MTEXT" || 
                               entity.dxfType === "ATTRIB" ||
                               entity.dxfType === "HATCH" ||
                               entity.dxfType === "SOLID" ||
                               entity.dxfType === "3DFACE";
        const key = new BatchingKey(entity.layer, blockCtx?.name,
