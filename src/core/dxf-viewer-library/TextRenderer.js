...
export class TextRenderer {
        ...
        for (const char of text) {
            ...
            if (!this.fontFetchers || this.fontFetchers.length === 0) {
                if (!missingChars.includes(char)) {
                    ...
                    console.warn(`[TextRenderer] No font fetchers available for character: ${char} (U+${char.codePointAt(0).toString(16).toUpperCase()})`)
                }
                charMissing = true
                continue
            }
            while (this.fontFetchers.length > 0) {
                try {
                    ...
                    console.warn(`[TextRenderer] Failed to fetch font for character '${char}':`, error)
                    // Continue to next font fetcher
                }
            }
            if (!found) {
                if (!missingChars.includes(char)) {
                    ...
                    console.warn(`[TextRenderer] Missing glyph for character: ${char} (U+${codePoint.toString(16).toUpperCase()})`)
                }
                charMissing = true
            }
        }
        
        if (charMissing && missingChars.length > 0) {
            console.warn(`[TextRenderer] Missing ${missingChars.length} character(s):`, missingChars.map(c => `'${c}' (U+${c.codePointAt(0).toString(16).toUpperCase()})`).join(', '))
        }
    ...
    *RenderMText({formattedText, position, fontSize, width = null, rotation = 0, direction = null,
    ...
    async _FetchFont(fontFetcher) {
        try {
            const fontData = await fontFetcher()
            if (!fontData) {
                throw new Error('Font fetcher returned null/undefined')
            }
            return new Font(fontData)
        } catch (error) {
            console.error('[TextRenderer] Error fetching font:', error)
            throw error
        }
    }
}
