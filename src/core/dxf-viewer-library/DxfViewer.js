...
import {MonochromeColorStrategy} from "./MonochromeColorStrategy.js"
...
export class DxfViewer {
    ...
    constructor(domContainer, options = null) {
        ...
        /* Initialize color transformation strategy.
         * Hard-coded to MonochromeColorStrategy for now.
         * Future: can be configured via options or switched at runtime.
         */
        this.colorStrategy = new MonochromeColorStrategy()
        ...
        /* Enable sorting to support renderOrder for interaction overlays. */
    ...
    _LoadBatch(scene, batch) {
        ...
        for (const obj of objects) {
            obj.userData.dxfHandle = batch.dxfHandle;
            obj.userData.dxfType = batch.dxfType;
            obj.userData.originalMaterial = obj.material;
            obj.userData.originalScale = obj.scale.clone();

            // Fix for Raycasting: Create 3D geometry if needed
            if (batch.dxfHandle && obj.geometry.getAttribute("position").itemSize === 2) {
                 ...
                 for (let i = 0; i < count; i++) {
                     const x = oldPos.getX(i);
                     const y = oldPos.getY(i);
                     if (isNaN(x) || isNaN(y)) {
                         newArr[i * 3] = 0;
                         newArr[i * 3 + 1] = 0;
                     } else {
                   ...
                   raycastGeo.setAttribute("position", new three.BufferAttribute(newArr, 3));
                 if (oldGeo.index) {
                     raycastGeo.setIndex(oldGeo.index);
                 }
                 
                 sanitizeBufferGeometry(raycastGeo);
                 
                 obj.userData.raycastGeometry = raycastGeo;
                 
                 obj.raycast = function(raycaster, intersects) {
                     const originalGeo = this.geometry;
                     this.geometry = this.userData.raycastGeometry;
    ...
    _TransformColor(color) {
        // Delegate to the active color strategy
        // Calculate background luminance for strategies that need it
        const backgroundLuminance = Luminance(this.clearColor)
        return this.colorStrategy.transformColor(color, backgroundLuminance)
    }
}
...
/**
 * Sanitizes a BufferGeometry by replacing any non-finite values (NaN, Infinity)
 * in the position attribute with 0. Also computes bounding volumes.
 * @param {three.BufferGeometry} geometry 
 */
function sanitizeBufferGeometry(geometry) {
    ...
    // Initialize min/max for manual bbox computation
    let minX = Infinity, minY = Infinity, minZ = Infinity;
    let maxX = -Infinity, maxY = -Infinity, maxZ = -Infinity;

    // Use a simple loop over the array to clean values and compute bounds
    // This avoids calling computeBoundingBox() which throws on NaNs
    for (let i = 0; i < array.length; i += itemSize) {
        // Check/Fix X
        let x = array[i];
        if (!isFinite(x)) { x = 0; array[i] = 0; sanitized = true; }
        
        // Check/Fix Y
        let y = (itemSize > 1) ? array[i+1] : 0;
        if (!isFinite(y)) { y = 0; array[i+1] = 0; sanitized = true; }
        
        // Check/Fix Z
        let z = (itemSize > 2) ? array[i+2] : 0;
        if (!isFinite(z)) { z = 0; array[i+2] = 0; sanitized = true; }
    ...
    if (minX === Infinity) { // Empty or all invalid
        minX = minY = minZ = 0;
        maxX = maxY = maxZ = 0;
    }
    ...
    // Manually compute bounding sphere from the box center and size
    // This avoids calling geometry.computeBoundingSphere() which can throw errors on some browsers/versions
    if (!geometry.boundingSphere) {
        geometry.boundingSphere = new three.Sphere();
    }
    geometry.boundingBox.getCenter(geometry.boundingSphere.center);
    
    // Calculate radius as half the diagonal of the box
    const vector = new three.Vector3();
    geometry.boundingBox.getSize(vector);
    geometry.boundingSphere.radius = vector.length() * 0.5;
}
...
class Batch {
    ...
    *_CreateObjects(instanceBatch) {
        ...
        function CreateObject(vertices, indices) {
            ...
            // CRITICAL: Sanitize geometry before use to prevent NaN errors
            sanitizeBufferGeometry(geometry);
